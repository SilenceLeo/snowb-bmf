...
- **TTC** (TrueType Collection) - 包含多个字体的集合文件。导入 TTC 文件时，SnowB BMF 会自动检测并弹出选择对话框，让您选择要加载的字体。

### 导入 TTC 文件

TTC（TrueType Collection）文件将多个相关字体打包在一个文件中，例如同一字体家族的不同字重或样式。SnowB BMF 为 TTC 文件提供了内置支持，具有直观的选择流程：

1. **自动检测**：当您导入 `.ttc` 文件时，应用程序会自动识别集合格式。
2. **字体选择对话框**：弹出一个对话框，列出集合中的每个字体。每个条目显示：
   - **全名 (Full Name)** — 完整的字体名称（例如，"Arial Bold"）。
   - **字体家族 (Font Family)** — 字体家族名称（例如，"Arial"）。
   - **子族 (Font Subfamily)** — 具体的样式或字重（例如，"Bold"）。
3. **多选支持**：使用复选框可以从集合中选择一个或多个字体。
4. **独立加载**：每个选中的字体会独立添加到字体列表中，允许您单独使用它们或将它们组合到回退链中。
...
当您使用 **"Add Font File"** 按钮导入字体时，应用程序会使用矢量渲染。这种先进的方法使用强大的 [fontkit](https://github.com/foliojs/fontkit) 库直接解析字体的轮廓数据。

**为什么选择 fontkit？** 与之前的字体解析方案相比，fontkit 提供了更广泛的格式兼容性、内置的 TTC/TrueType Collection 支持以及更好的可变字体处理能力——使其成为需要处理各种字体文件的位图字体生成器的理想选择。
...
3.  **善用 TTC 文件**：如果您有包含整个字体家族的 TTC 文件，只需导入一次并选择您需要的字重/样式即可。
4.  **从 80% 的锐化开始**：这是一个很好的起点。根据您的目标字体大小和显示分辨率进行调整。
5.  **测试字符覆盖范围**：确保您的字体组合覆盖所有必需的字符（例如，拉丁文、中日韩文、符号）。
6.  **在目标尺寸下预览**："锐化"设置的效果在最终的目标分辨率下最为明显。
...
**为什么我的字体导入失败了？**
- 请仔细检查文件是否为支持的格式（TTF、OTF、WOFF 或 TTC），并确保文件没有损坏。

**我导入了 TTC 文件但什么都没发生？**
- 请确保在选择对话框中至少选中了一个字体，然后再点击"加载"。如果对话框没有出现，文件可能不是有效的 TTC 集合。
